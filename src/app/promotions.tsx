...
function formatPromotionValue(item: Promotion) {
  ...
  if (item.type === 'bonus_points') {

    return `+${item.bonusPoints ?? 0} điểm`;

  }

  if (item.type === 'percentage') {

    return `Giảm ${item.discountValue ?? 0}%`;

  }

  if (item.type === 'fixed_amount') {

    return `Giảm ${(item.discountValue ?? 0).toLocaleString('vi-VN')} đ`;
      ...
      <Screen>
        ...
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>
            ...
            Hiện tại app mobile chỉ hiển thị khuyến mãi đang áp dụng cho customer.
  ...
  return (
    ...
    >
      ...
      <View style={styles.hero}>
        ...
        <Text style={styles.heroText}>Kiểm tra điều kiện trước khi đặt lịch để tận dụng ưu đãi tốt nhất.</Text>
      ...
      {items.map((item) => (
        ...
        <View key={item._id} style={styles.card}>
          ...
          <View style={styles.metaBlock}>
            ...
            {typeof item.minSpend === 'number' && item.minSpend > 0 ? (

              <Text style={styles.metaText}>

                Đơn tối thiểu: {item.minSpend.toLocaleString('vi-VN')} đ
