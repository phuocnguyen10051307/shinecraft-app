...
const TabIcon = ({ name, color }: { name: SymbolViewProps['name']; color: ColorValue }) => <SymbolView name={name} size={23} tintColor={color} />;
...
export default function ProtectedTabsLayout() {
  ...
  return <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: colors.primary, tabBarInactiveTintColor: colors.muted, tabBarStyle: styles.tabBar, tabBarLabelStyle: styles.label }}>
    ...
    <Tabs.Screen name={'service-histories'} options={{ href: user?.role === 'customer' ? undefined : null, title: 'Lich su', tabBarIcon: ({ color }) => <TabIcon color={color} name={{ ios: 'clock.arrow.circlepath', android: 'history', web: 'history' }} /> }} />
...
const styles = StyleSheet.create({ tabBar: { height: 68, paddingTop: 7, paddingBottom: 8, borderTopColor: colors.border, backgroundColor: colors.surface }, label: { fontSize: 11, fontWeight: '700' } });
